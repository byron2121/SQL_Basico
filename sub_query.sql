...
create table if not exists mesvsgastos Select m.nombre_mes, g.gastos_mes from byron.mes m 
inner join  byron.gastos g on (m.id_mes=g.id_mes) 
order by gastos_mes DESC ;


select* from byron.mesvsgastos;

#SUB CONSULTAS (SUB_QUERY)
select* from byron.mesvsgastos where gastos_mes>(select avg(gastos_mes) from byron.mesvsgastos);
